...
- CLI per README: `epub-pdf-wrap <input.pdf> [-o <output.epub>] [-r <num>]`; `-r` is target render width in pixels (default = PDF's own resolution); default output = input name with `.epub` extension.
